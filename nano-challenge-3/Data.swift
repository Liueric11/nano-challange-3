...
class ModelData: ObservableObject, Identifiable {
    ...
    init(day: Date, sales: Int, selectedMonth: String? = nil, selectedCost: Double? = nil) {
        self.day = day
        self.sales = sales
...
struct SalesData {
    ...
    static var last7DaysTotal: Int {
        last7Days.map { $0.sales }.reduce(0, +)
    }

    static var last7DaysAverage: Double {
        Double(last7DaysTotal / last7Days.count)
    }
    ...
    static let last365Days: [ModelData] = stride(from: 0, to: 200, by: 1).compactMap {
            ...
            return ModelData(day: day, sales: sales)
        }
    ...
    static var last1MonthTotal: Int {
        last1Month.map { $0.sales }.reduce(0, +)
    }

    static var last1MonthDailyAverage: Int {
        last1Month.map { $0.dailyAverage }.reduce(0, +) / last1Month.count
    }
}
